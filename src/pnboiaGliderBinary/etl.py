...
class GliderDataToDB():
    ...
    def __init__(self,
        ...
        self.mission_csv = self.get_mission_registring_csv()

        self.eng_params_selection = self.get_parameters(parameter_type="ENG")
        self.sci_params_selection = self.get_parameters(parameter_type="SCI")


    def get_mission_registring_csv(self):
        mission_registring_file_path = os.path.join(self.binary_files_path, "glider_mission_registring.csv")
        mission = pd.read_csv(mission_registring_file_path)
        mission = mission.set_index("CAMPO")
        return mission

    def mission_registring_check_db(self):
        mission = self.db.get(table='glider.missions')

        if self.mission_csv.loc["name"].values[0] in mission.name.values:
            return True
        else:
            return False

    def mission_registring_to_db(self):
        if not self.mission_registring_check_db():
            self.db.post(data=self.mission_csv.T, schema='glider', table='missions')


    def get_parameters(self, parameter_type:str):
        parameters_table = self.db.get(table='glider_data.parameters',
                                            type=["=", parameter_type])
        return parameters_table.name.values
